...
class upload {
    ...
    public function up($f_input) {
        // $_FILES里面有没有$name指定的单元
        if(!isset($_FILES[$f_input])) {
            return false;
        }
        $f = $_FILES[$f_input];
		$pic = Array();
		foreach($f['error'] as $key=>$error ){
			// 判断错误码,是否上传成功
			if($this->errno = $error > 0) {
				return $pic;
			}
			// 判断大小
			if(!$this->checkSize($f['size'][$key])) {
				$this->errno = 8;
				return $pic;
			}
			// 判断类型
			$ext = $this->getExt($f['name'][$key]);
			...
			if(!move_uploaded_file($f['tmp_name'][$key],$path)) {
				$this->errno = 11;
				return $pic;
			}
			$path = str_replace(ROOT,'',$path);
			$pic[$key]['Bg_name'] = $f['name'][$key];
			$pic[$key]['Bg_url'] = $path;
		}
		return $pic;
    }
